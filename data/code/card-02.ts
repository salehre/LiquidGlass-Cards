 ...
 * (baseFrequency 0.018, scale 120) — a stronger, more "fluid" distortion
 * than Liquid Crystal's shared filter. Because the params differ from
 * card01's, this filter is defined locally (not in app.vue) and referenced
 * via `filter: url(#glass-distortion-02)`.
 */
export const card02Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-02 filter (different
     baseFrequency/scale than Liquid Crystal), so the <svg><defs><filter>
...
<div class="fluid-amber-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.fluid-amber-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
...
.fluid-amber-card::after {
  ...
  pointer-events: none;
    ...
    bootstrap: `<!-- This card ships its own #glass-distortion-02 filter — see the
...
<div class="fluid-amber-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.fluid-amber-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.fluid-amber-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-02 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-02" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="fluid-amber-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.fluid-amber-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.fluid-amber-card::before {
  content: '';
  position: absolute;
  inset: 0;
