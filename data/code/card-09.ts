...
/**
 * Plasma Flow — glass card with its own #glass-distortion-09 SVG filter
 * (baseFrequency 0.035, scale 180) — the highest baseFrequency and scale
 * in the whole set, paired with a light 4px backdrop blur for a sharp,
 * energetic warp. Because the params differ from the other cards, this
 * filter is defined locally (not in app.vue) and referenced via
 * `filter: url(#glass-distortion-09)`.
 */
export const card09Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-09 filter (different
...
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-09" x="0%" y="0%" width="100%" height="100%">
      <feTurbulence type="fractalNoise" baseFrequency="0.035 0.035" numOctaves="2" seed="92" result="noise" />
      <feGaussianBlur in="noise" stdDeviation="2" result="blurred" />
      <feDisplacementMap in="SourceGraphic" in2="blurred" scale="180" xChannelSelector="R" yChannelSelector="G" />
    </filter>
  </defs>
</svg>

<div class="plasma-flow-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.plasma-flow-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.plasma-flow-card::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
  border-radius: 28px;
  box-shadow: inset 0 0 3px -10px rgba(255, 255, 255, 0.7);
  pointer-events: none;
}
.plasma-flow-card::after {
  content: '';
  position: absolute;
  inset: 0;
  z-index: -1;
  ...
  filter: url(#glass-distortion-09);
  -webkit-filter: url(#glass-distortion-09);
  isolation: isolate;
  pointer-events: none;
}
</style>`,
    bootstrap: `<!-- This card ships its own #glass-distortion-09 filter — see the
...
<div class="plasma-flow-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.plasma-flow-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.plasma-flow-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-09 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-09" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="plasma-flow-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.plasma-flow-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.plasma-flow-card::before {
  content: '';
  position: absolute;
  inset: 0;
