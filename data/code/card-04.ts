 ...
 * (baseFrequency 0.008, scale 150) — a broad, heavy, mercury-like wobble
 * instead of a fine ripple. Because the params differ from the other
 * cards, this filter is defined locally (not in app.vue) and referenced
 * via `filter: url(#glass-distortion-04)`.
 */
export const card04Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-04 filter (different
...
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-04" x="0%" y="0%" width="100%" height="100%">
...
<div class="mercury-drop-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.mercury-drop-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.mercury-drop-card::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
...
.mercury-drop-card::after {
  ...
  filter: url(#glass-distortion-04);
  -webkit-filter: url(#glass-distortion-04);
  isolation: isolate;
  pointer-events: none;
}
</style>`,
    bootstrap: `<!-- This card ships its own #glass-distortion-04 filter — see the
...
<div class="mercury-drop-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.mercury-drop-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.mercury-drop-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-04 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-04" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="mercury-drop-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.mercury-drop-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.mercury-drop-card::before {
  content: '';
  position: absolute;
  inset: 0;
