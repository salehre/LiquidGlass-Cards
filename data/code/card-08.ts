...
/**
 * Silk Veil — glass card with its own #glass-distortion-08 SVG filter
 * (baseFrequency 0.006, scale 45) — the lowest-strength distortion in the
 * set, paired with a heavier 18px backdrop blur for a soft, smooth,
 * silky look. Because the params differ from the other cards, this
 * filter is defined locally (not in app.vue) and referenced via
 * `filter: url(#glass-distortion-08)`.
 */
export const card08Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-08 filter (different
...
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-08" x="0%" y="0%" width="100%" height="100%">
...
<div class="silk-veil-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.silk-veil-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.silk-veil-card::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
...
.silk-veil-card::after {
  ...
  filter: url(#glass-distortion-08);
  -webkit-filter: url(#glass-distortion-08);
  isolation: isolate;
  pointer-events: none;
}
</style>`,
    bootstrap: `<!-- This card ships its own #glass-distortion-08 filter — see the
...
<div class="silk-veil-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.silk-veil-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.silk-veil-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-08 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-08" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="silk-veil-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.silk-veil-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.silk-veil-card::before {
  content: '';
  position: absolute;
  inset: 0;
