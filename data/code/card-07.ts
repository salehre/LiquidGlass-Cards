...
/**
 * Molten Glass — glass card with its own #glass-distortion-07 SVG filter
 * (baseFrequency 0.03, scale 140) — the highest baseFrequency in the set
 * combined with a strong scale, for a hot, roiling distortion. Because
 * the params differ from the other cards, this filter is defined locally
 * (not in app.vue) and referenced via `filter: url(#glass-distortion-07)`.
 */
export const card07Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-07 filter (different
...
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-07" x="0%" y="0%" width="100%" height="100%">
      <feTurbulence type="fractalNoise" baseFrequency="0.03 0.03" numOctaves="2" seed="92" result="noise" />
...
<div class="molten-glass-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.molten-glass-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.molten-glass-card::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
...
.molten-glass-card::after {
  ...
  filter: url(#glass-distortion-07);
  -webkit-filter: url(#glass-distortion-07);
  isolation: isolate;
  pointer-events: none;
}
</style>`,
    bootstrap: `<!-- This card ships its own #glass-distortion-07 filter — see the
...
<div class="molten-glass-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.molten-glass-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.molten-glass-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-07 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-07" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="molten-glass-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.molten-glass-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.molten-glass-card::before {
  content: '';
  position: absolute;
  inset: 0;
