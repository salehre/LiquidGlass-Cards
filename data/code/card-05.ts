...
/**
 * Ocean Wave — glass card with its own #glass-distortion-05 SVG filter
 * (baseFrequency 0.015, scale 95) — a mid-strength wave, between Ice
 * Ripple's tight ripple and Fluid Amber's broader wobble. Because the
 * params differ from the other cards, this filter is defined locally
 * (not in app.vue) and referenced via `filter: url(#glass-distortion-05)`.
 */
export const card05Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-05 filter (different
...
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-05" x="0%" y="0%" width="100%" height="100%">
      <feTurbulence type="fractalNoise" baseFrequency="0.015 0.015" numOctaves="2" seed="92" result="noise" />
      <feGaussianBlur in="noise" stdDeviation="2" result="blurred" />
      <feDisplacementMap in="SourceGraphic" in2="blurred" scale="95" xChannelSelector="R" yChannelSelector="G" />
    </filter>
  </defs>
</svg>

<div class="ocean-wave-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.ocean-wave-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.ocean-wave-card::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
...
.ocean-wave-card::after {
  ...
  filter: url(#glass-distortion-05);
  -webkit-filter: url(#glass-distortion-05);
  isolation: isolate;
  pointer-events: none;
}
</style>`,
    bootstrap: `<!-- This card ships its own #glass-distortion-05 filter — see the
...
<div class="ocean-wave-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.ocean-wave-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.ocean-wave-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-05 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-05" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="ocean-wave-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.ocean-wave-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.ocean-wave-card::before {
  content: '';
  position: absolute;
  inset: 0;
