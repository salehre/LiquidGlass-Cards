...
/**
 * Crystal Mist — glass card with its own #glass-distortion-06 SVG filter
 * (baseFrequency 0.02, scale 110) — sits between Ocean Wave and Fluid
 * Amber in strength. Because the params differ from the other cards,
 * this filter is defined locally (not in app.vue) and referenced via
 * `filter: url(#glass-distortion-06)`.
 */
export const card06Code: CardFrameworkCode = {
    tailwind: `<!-- This card ships its own #glass-distortion-06 filter (different
...
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-06" x="0%" y="0%" width="100%" height="100%">
...
<div class="crystal-mist-card relative w-full max-w-[400px] h-[300px] rounded-[28px] isolate cursor-pointer">
...
<style>
.crystal-mist-card { box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.crystal-mist-card::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
  border-radius: 28px;
  box-shadow: inset 0 0 8px -2px rgba(255, 255, 255, 0.7);
  pointer-events: none;
}
.crystal-mist-card::after {
  content: '';
  position: absolute;
  inset: 0;
  z-index: -1;
  ...
  filter: url(#glass-distortion-06);
  -webkit-filter: url(#glass-distortion-06);
  isolation: isolate;
  pointer-events: none;
}
</style>`,
    bootstrap: `<!-- This card ships its own #glass-distortion-06 filter — see the
...
<div class="crystal-mist-card position-relative rounded-4 mx-auto" style="width: 400px; max-width: 100%; height: 300px;">
...
<style>
.crystal-mist-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3); }
.crystal-mist-card::before {
  content: '';
  position: absolute;
  inset: 0;
...
.glass-button:focus { box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.4); }
    ...
    vuetify: `<!-- This card ships its own #glass-distortion-06 filter — see the
     <svg><defs><filter> block below. -->
<svg width="0" height="0" style="position: absolute">
  <defs>
    <filter id="glass-distortion-06" x="0%" y="0%" width="100%" height="100%">
...
<v-card class="crystal-mist-card pa-6" rounded="xl" elevation="0" width="400" height="300" style="max-width: 100%;">
...
<style>
.crystal-mist-card { isolation: isolate; box-shadow: 0px 0px 21px -8px rgba(255, 255, 255, 0.3) !important; background: transparent !important; }
.crystal-mist-card::before {
  content: '';
  position: absolute;
  inset: 0;
